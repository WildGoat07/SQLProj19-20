/* question 7 */
SELECT DISTINCT q.no_question AS
"Les questions auxquelles les utilisateurs n'ont jamais répondu lors de leurs différentes sessions"
FROM `question` AS q
WHERE q.no_question NOT IN (
    SELECT rd.no_question
    FROM `rep_donnee` AS rd
    )
;
...
/* question 12 */
SELECT q.no_question AS "Des questions ne comportant aucune bonne réponse"
FROM `question` AS q
WHERE q.no_question NOT IN (
    SELECT rp.no_question
    FROM `question` AS q, `rep_proposee` AS rp
    WHERE q.no_question=rp.no_question
        AND rp.etat_rep="1"
    )
